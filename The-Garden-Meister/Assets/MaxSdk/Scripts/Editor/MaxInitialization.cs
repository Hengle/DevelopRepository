...
public class MaxInitialize
{
    ...
    static MaxInitialize()
    {
        ...
        // Check if obsolete scripts are in the project
        var iOSPostProcessorFile = Path.Combine("Assets", "MaxSdk/Scripts/Editor/MaxPostProcessBuildiOS.cs");
        if (CheckExistence(iOSPostProcessorFile))
        {
            Debug.Log("Deleting obsolete script at path: " + iOSPostProcessorFile);
            FileUtil.DeleteFileOrDirectory(iOSPostProcessorFile);
            changesMade = true;
        }
