...
public class Rope : MonoBehaviour
{
    ...
    {
        Refresh();
        //transform.Find("6").GetComponent<Rigidbody>().AddForce(Vector3.right * 5f, ForceMode.Impulse);
    }
    ...
    void Grap(Transform transform, Collider other)
    {
        if (!isGrap)
        {
            ...
            other.gameObject.transform.localEulerAngles = Vector3.zero + new Vector3(0f, 180f, 0f);

            foreach (var c in this.transform.GetComponentsInChildren<RopeCollider>())
            {
                c.OnTriggerListener -= Grap;
    ...
    void Cut(Transform transform, Collision other)
    {
        ...
        if (!isCut)
        {
            ...
            eventParams["FailedType"] = "RopeCut";
