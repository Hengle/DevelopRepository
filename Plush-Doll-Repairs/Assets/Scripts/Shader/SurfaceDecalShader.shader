ShaShader "Custom/SurfaceDecalShader-HSV"
{
	Properties{
...
void surf(Input IN, inout SurfaceOutputStandard o) {
	...
	fixed2 pos = fixed2((IN.uv_MainTex.x - _BlendStartU) * 1 / u_scale,
		(IN.uv_MainTex.y - _BlendStartV) * 1 / v_scale);
	float condition = step(IN.uv_MainTex.x, _BlendEndU) *
		step(IN.uv_MainTex.y, _BlendEndV) *
		step(_BlendStartU, IN.uv_MainTex.x) *
		step(_BlendStartV, IN.uv_MainTex.y);

	fixed4 c = tex2D(_MainTex, IN.uv_MainTex);
	half4 d = tex2D(_DecalTex, pos);
