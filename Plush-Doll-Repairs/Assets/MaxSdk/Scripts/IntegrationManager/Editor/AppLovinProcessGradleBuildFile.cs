...
public abstract class AppLovinProcessGradleBuildFile
{
    ...
    protected void AddAppLovinQualityServicePlugin(string applicationGradleBuildFilePath)
    {
        ...
        var sanitizedLines = RemoveLegacyPlugin(lines);
    ...
    private static List<string> RemoveLegacyPlugin(List<string> lines)
    {
        ...
        {
            if (!legacyPluginMatched && line.Contains(SafeDkLegacyPlugin))
            {
            ...
            if (!legacyRepoRemoved && line.Contains(SafeDkLegacyMavenRepo))
            {
                legacyRepoRemoved = true;
                continue;
            }

            if (!legacyDependencyClassPathRemoved && line.Contains(SafeDkLegacyDependencyClassPath))
            {
                legacyDependencyClassPathRemoved = true;
                continue;
            }

            if (!legacyPluginRemoved && TokenSafeDkLegacyApplyPlugin.IsMatch(line))
            {
