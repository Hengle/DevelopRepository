...
public class AppLovinPostProcessiOS
{
    ...
    public static void OnPostProcessBuild(BuildTarget buildTarget, string buildPath)
    {
        ...
        {
            Debug.LogError("Failed to install Quality Service plugin. SDK Key is empty. Please enter the AppLovin SDK Key in the Integration Manager.");
            return;
        }
        ...
        // Ruby is installed, run `ruby AppLovinQualityServiceSetup-xyz_iOS.rb`
        var result = AppLovinCommandLine.Run("ruby", OutputFileName, buildPath);

        // Check if we have an error.
        if (result.ExitCode != 0)
        {
            Debug.LogError("Failed to set up AppLovin Quality Service for your project with error: " + result.StandardError);
            return;
        }
