...
namespace RolePlayingGame
{
    public class Player : MonoBehaviour,IDamageable<int>
    {
        ...
        {
            //ステータスの設定
            charaStatus = new CharaStatus(5, 5, 1, 5, 1, 0, map, map.GetGridPos(Camera.main.WorldToScreenPoint(transform.position)));
            charaStatus.Map.ChangeMapData(charaStatus.GridPos.x, charaStatus.GridPos.y, ChipType.Player);
        }
        ...
        {
            if((ChipType)charaStatus.Map.CallMapData(charaStatus.MapPosFront()) == ChipType.Enemy)
            {
                Debug.Log("エンカウント");
        ...
        IEnumerator Move(Vector3 beforePos, float beforeTime)
        {
            ...
            charaStatus.GridPos = charaStatus.Map.MapPosUpdate(charaStatus.GridPos, Camera.main.WorldToScreenPoint(transform.position), ChipType.Player);
        ...
        private void Attack()
        {
            var target = charaStatus.MapPosFront();

            RaycastHit2D hit = Physics2D.Raycast(new Vector2(transform.position.x, transform.position.y) + ChengeVector(charaStatus.Dir), new Vector2(0f,-1.0f), 5);
            Debug.DrawRay(new Vector2(transform.position.x, transform.position.y) + ChengeVector(charaStatus.Dir), new Vector2(0f, -1.0f), Color.red, 10);
            var damageable = hit.collider.gameObject.GetComponent<IDamageable<int>>();
            Debug.Log(damageable);
            if (damageable != null)
            {
                damageable.Damage(charaStatus.GetStatus(CharaStatusType.AP));
            }

            //var enemys = GameObject.FindGameObjectsWithTag("Enemy");
            //foreach (var enemy in enemys)
            //{
            //    var enemyScript = enemy.transform.GetComponent<Enemy>();
            //    if(enemyScript.GridPosition == target)
            //    {
            //        enemyScript.Damage(charaStatus.GetStatus(CharaStatusType.AP));
            //    }
            //}
